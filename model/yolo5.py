...
class Head(nn.Module):
    def __init__(self, anchors, strides, num_cls, train=False):
        super(Head, self).__init__()

        channels = 3 * (5 + num_cls)
        anchors = anchors.view(len(anchors), -1, 2)
        ...
        self.anchor = Anchor(anchors, strides, num_cls, train)
...
class YOLO5(nn.Module):
    def __init__(self, anchors, strides, num_cls, device, train):
        super().__init__()
        ...
        self.head = Head(anchors, strides, num_cls, train).to(device)
